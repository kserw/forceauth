...
export function PermissionSetsPanel() {
  ...
  const totalPages = Math.ceil(permissionSets.length / ITEMS_PER_PAGE);
  const paginatedSets = permissionSets.slice(currentPage * ITEMS_PER_PAGE, (currentPage + 1) * ITEMS_PER_PAGE);
  ...
  return (
    <div className="h-full p-4 rounded-md border border-[hsl(var(--border))] bg-[hsl(var(--card))] flex flex-col">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-[hsl(var(--muted-foreground))] tabular-nums">
            {isLoading ? <Loader2 className="w-3 h-3 animate-spin inline" /> : `${permissionSets.length} sets`}
          </span>
          <button
            onClick={loadData}
            disabled={isLoading}
      ...
      <div ref={scrollRef} className="flex-1 overflow-auto -mx-4 px-4">
        {isLoading && permissionSets.length === 0 ? (
          ...
          <div className="space-y-1">
            {paginatedSets.map((ps) => {
              const psUrl = getSalesforcePermissionSetUrl(instanceUrl, ps.id);
              return (
                <div
                  key={ps.id}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between pt-3 mt-3 border-t border-[hsl(var(--border))]">
          <span className="text-[10px] text-[hsl(var(--muted-foreground))]">
            {currentPage * ITEMS_PER_PAGE + 1}-{Math.min((currentPage + 1) * ITEMS_PER_PAGE, permissionSets.length)} of {permissionSets.length}
          </span>

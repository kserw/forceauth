...
export function InstalledPackagesPanel() {
  ...
  const paginatedPackages = packages.slice(currentPage * ITEMS_PER_PAGE, (currentPage + 1) * ITEMS_PER_PAGE);
    ...
    return (
      <div className="h-full p-4 rounded-md border border-[hsl(var(--border))] bg-[hsl(var(--card))] flex flex-col">
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs text-[hsl(var(--muted-foreground))]">// installed_packages[]</span>
        </div>
        <div className="flex-1 flex items-center justify-center">
          <span className="text-xs text-[hsl(var(--muted-foreground))]">login to view packages</span>
        </div>
      </div>
    );
  }
    ...
    <div className="h-full p-4 rounded-md border border-[hsl(var(--border))] bg-[hsl(var(--card))] flex flex-col">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-[hsl(var(--muted-foreground))] tabular-nums">
            {isLoading ? <Loader2 className="w-3 h-3 animate-spin inline" /> : `${packages.length} packages`}
          </span>
          <button
            onClick={loadData}
            disabled={isLoading}
      ...
      <div ref={scrollRef} className="flex-1 overflow-auto -mx-4 px-4">
        {isLoading && packages.length === 0 ? (
          ...
          <div className="flex items-center justify-center h-full">
            <span className="text-xs text-[hsl(var(--muted-foreground))]">no packages installed</span>
          </div>
        ) : (
          <div className="space-y-1">
            {paginatedPackages.map((pkg) => {
              const packageUrl = getSalesforceInstalledPackageUrl(instanceUrl, pkg.id);
              return (
                <div
                  key={pkg.id}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-[hsl(var(--foreground))] truncate">{pkg.name}</span>
                      {pkg.namespace && (
                  ...
                  <div className="text-right">
                    {pkg.version && (
                      <span className="text-xs text-[hsl(var(--foreground))] font-mono">v{pkg.version}</span>
                    )}
                  </div>
                </div>
              );
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between pt-3 mt-3 border-t border-[hsl(var(--border))]">
          <span className="text-[10px] text-[hsl(var(--muted-foreground))]">
            {currentPage * ITEMS_PER_PAGE + 1}-{Math.min((currentPage + 1) * ITEMS_PER_PAGE, packages.length)} of {packages.length}
          </span>

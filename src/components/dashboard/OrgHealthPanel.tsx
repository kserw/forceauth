...
import { Loader2, RefreshCw, Zap, Users } from 'lucide-react';
...
function getUsageColor(percentage: number) {
  if (percentage >= 90) return { ring: 'stroke-[hsl(var(--destructive))]', text: 'text-[hsl(var(--destructive))]' };
  if (percentage >= 70) return { ring: 'stroke-[hsl(var(--warning))]', text: 'text-[hsl(var(--warning))]' };
  return { ring: 'stroke-[hsl(var(--success))]', text: 'text-[hsl(var(--success))]' };
}
...
function Gauge({ used, total, label, icon }: GaugeProps) {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="mt-2 flex items-center gap-1.5 text-[hsl(var(--muted-foreground))]">
        {icon}
        <span className="text-[10px] uppercase tracking-wide">{label}</span>
      </div>

      <div className="mt-1 text-sm text-[hsl(var(--foreground))] tabular-nums">
...
export function OrgHealthPanel() {
  ...
  // Get Salesforce license (the main one)
